        ...
        <div class="max-w-4xl mx-auto">
            ...
            <form action="{{ route('jobs.update', $job->id) }}" method="POST" class="space-y-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select name="type"
                                ...
                                <option value="full-time"
                                    {{ old('type', $job->type ?? '') == 'full-time' ? 'selected' : '' }}>Full Time</option>
                                <option value="part-time"
                                    {{ old('type', $job->type ?? '') == 'part-time' ? 'selected' : '' }}>Part Time</option>
                                <option value="contract"
                                    {{ old('type', $job->type ?? '') == 'contract' ? 'selected' : '' }}>Contract</option>
                                <option value="internship"
                                    {{ old('type', $job->type ?? '') == 'internship' ? 'selected' : '' }}>Internship
                                </option>
                        ...
                        <div>
                            ...
                            <div class="flex items-center p-1 bg-slate-100/50 rounded-2xl border border-slate-200">
                                ...
                                <input type="number" name="experience_min" placeholder="Min"
                                    value="{{ old('experience_min', $job->experience_min ?? '') }}"
                ...
                <div class="bg-white rounded-[2rem] shadow-sm border border-slate-200/60 overflow-hidden">
                    ...
                    <div class="p-8 space-y-8">
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="4"
                                ...
                                placeholder="Describe the mission, culture, and day-to-day impact...">{{ old('description', $job->description ?? '') }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="requirements" rows="4"
                                class="block w-full px-4 py-4 bg-slate-50/50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all resize-none"
                                placeholder="e.g. 3+ years of Laravel experience...">{{ old('requirements', $job->requirements ?? '') }}</textarea>
                        </div>
                    ...
                    {{ old('status', $job->status ?? 'inactive') === 'active' ? 'text-indigo-600' : 'text-gray-400' }}">

                                    {{ old('status', $job->status ?? 'inactive') === 'active' ? 'Active' : 'Inactive' }}
                                </span>
                                ...
                                <label class="relative inline-flex items-center cursor-pointer">
                                    ...
                                    <input type="checkbox" name="status" value="active" class="sr-only peer"
                                        id="status-toggle"
                                        {{ old('status', $job->status ?? 'inactive') === 'active' ? 'checked' : '' }}>
                            ...
                            <div
                                ...
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" name="has_test" value="1" id="has_test"
                                        class="sr-only peer"
                                        {{ old('has_test', $job->has_test ?? false) ? 'checked' : '' }}>
                                    <div
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <label class="relative cursor-pointer">
                                        <input type="radio" name="test_mode" value="strict" class="sr-only peer"
                                            {{ old('test_mode', $job->test_mode ?? '') == 'strict' ? 'checked' : '' }}>
            ...
            peer-checked:border-indigo-600 peer-checked:bg-indigo-50/30">
                                    ...
                                    <!-- FLEXIBLE -->
                                    <label class="relative cursor-pointer">
                                        <input type="radio" name="test_mode" value="flexible" class="sr-only peer"
                                            {{ old('test_mode', $job->test_mode ?? 'flexible') == 'flexible' ? 'checked' : '' }}>
                                        ...
                                        <div class="flex p-1 bg-slate-100 rounded-xl">
                                            <label class="cursor-pointer">
                                                <input type="radio" name="question_type" value="subjective"
                                                    class="sr-only peer"
                                                    {{ old('question_type', 'subjective') == 'subjective' ? 'checked' : '' }}>
                                                <span
                                            ...
                                            <label class="cursor-pointer">
                                                <input type="radio" name="question_type" value="objective"
                                                    class="sr-only peer"
                                                    {{ old('question_type') == 'objective' ? 'checked' : '' }}>
                                                <span
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const testToggle = document.getElementById('has_test');
    const testOptions = document.getElementById('test-options');
    const container = document.getElementById('questions-container');
    const addBtn = document.getElementById('add-question');

    // ================= DATA (EDIT MODE) =================
    const hasTest = @json($job->has_test ?? false);
    const questions = @json($job->questions ?? []);
    ...
    // ================= TEST TOGGLE =================
    function showTestSection(show) {
        if (show) {
            testOptions.classList.remove('hidden');
            setTimeout(() => testOptions.classList.remove('opacity-0'), 10);
        } else {
            testOptions.classList.add('opacity-0');
            setTimeout(() => testOptions.classList.add('hidden'), 300);
        }
    }
        ...
        if (type === 'subjective') {
            ...
            div.innerHTML = `
                <div class="flex justify-between mb-4">
                    ...
                    <button type="button" class="remove text-slate-300 hover:text-red-500">✕</button>
                </div>
